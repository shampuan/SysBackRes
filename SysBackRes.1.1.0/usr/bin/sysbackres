...
class OperationThread(QThread):
    ...
    def run(self):
        ...
        try:
            if self.operation_type == 'BACKUP_IMAGE':
                # BURASI KRİTİK: total_size burada gönderilmeli
                success = self._execute_backup_image_flow(total_size)
            elif self.operation_type in ['BACKUP_CLONE', 'RESTORE_DISK_TO_DISK', 'RESTORE_IMAGE_TO_DISK', 'VERIFY_TWO_DISKS']:
                # BURASI KRİTİK: total_size burada gönderilmeli
                success = self._execute_ddrescue_flow(total_size)
    ...
    def _get_disk_size(self, dev_path):
        """Disk/Dosya boyutunu kesin olarak byte cinsinden döndürür."""
        # Eğer normal bir dosyaysa (İmaj dosyası)
        if os.path.exists(dev_path) and os.path.isfile(dev_path):
            return os.path.getsize(dev_path)
        
        # Eğer bir blok aygıtıysa (/dev/sdX)
        if dev_path.startswith('/dev/'):
            try:
                # lsblk ile byte cinsinden boyut (En güvenilir yöntem)
                cmd = ['lsblk', '-b', '-n', '-o', 'SIZE', dev_path]
                result = subprocess.run(cmd, capture_output=True, text=True, check=True)
                return int(result.stdout.strip().split('\n')[0])
            except:
                try:
                    # Alternatif: blockdev (pkexec gerektirebilir)
                    cmd = ['pkexec', 'blockdev', '--getsize64', dev_path]
                    result = subprocess.run(cmd, capture_output=True, text=True, check=True)
    ...
    def _execute_ddrescue_with_progress(self, cmd, description, total_size):
        """ddrescue'nun ANSI kaçış dizili çıktılarını temizler ve parse eder."""
        self.output_update.emit(f"[{time.strftime('%H:%M:%S')}] {description} başlatılıyor...")
        
        # Regex: 'ipos:' (input position) değerini yakalar
        progress_re = re.compile(r'ipos:\s*(\d+)\s*([kMG]?B)')
        # ANSI kaçış dizilerini temizlemek için regex
        ansi_re = re.compile(r'\x1b\[[0-9;]*[a-zA-Z]')

        try:
            process = subprocess.Popen(['pkexec'] + cmd, 
                                       stdout=subprocess.PIPE, 
            ...
            while True:
                ...
                # ddrescue terminali güncellemek için \r kullanır
                if char in ('\r', '\n'):
                    # ANSI kodlarını temizle ve satırı temizle
                    clean_line = ansi_re.sub('', buffer).strip()
                    
                    if clean_line:
                        # ipos: 5177 kB gibi verileri yakala
                        ...
                        if match:
                            ...
                            # Birimi byte'a çevir
                            bytes_read = raw_value
                            if unit == 'kB': bytes_read *= 1024
                            elif unit == 'MB': bytes_read *= 1024**2
                            elif unit == 'GB': bytes_read *= 1024**3
                        ...
                        # Sadece önemli aşama bilgilerini loga yaz
                        if "Pass" in clean_line or "Finished" in clean_line:
                            self.output_update.emit(clean_line)
    ...
    def _calculate_file_hash(self, filepath, hash_algo='sha512', block_size=1048576):
        """Dosyanın hash değerini hesaplar ve ilerleme çubuğunu günceller."""
        self.output_update.emit(f"[{time.strftime('%H:%M:%S')}] Hash Hesaplama başlatılıyor: {filepath}")
        
        if not os.path.exists(filepath):
            self.error_signal.emit(f"Hata: Dosya bulunamadı: {filepath}")
            return None
        ...
        try:
            with open(filepath, 'rb') as afile:
                ...
                while True:
                    ...
                    if file_size > 0:
                        ...
                        # Her blok okumada arayüzü güncelle
                        self.progress_update.emit(percentage) 
            ...
            self.error_signal.emit(f"Hata: Dosya/Aygıt bulunamadı: {filepath}")
            return None
        except InterruptedError as e:
            self.output_update.emit(str(e))
            raise
        except Exception as e:
            self.error_signal.emit(f"Hash hesaplama sırasında kritik hata: {str(e)}")
            return None


    # --- ANA OPERASYON AKIŞLARI ---

    def _execute_backup_image_flow(self, total_size):
        """Disk -> İmaj operasyonu (Hash hesapla ve Klonla)."""
        source_dev = self._parse_disk_selection(self.source)
        target_path = self.target 
        hash_file_path = target_path + ".sha512" 
        
        # --- ADIM 1: HASH HESAPLAMA (Progress Bar Busy Modda) ---
        self.output_update.emit(f"[{time.strftime('%H:%M:%S')}] Adım 1/2: Kaynak disk için SHA512 hash oluşturuluyor...")
        
        # ProgressBar'ı busy (sağa-sola giden) moda sok
        self.progress_update.emit(-1)
        
        # DÜZELTME: pkexec ile sha512sum çalıştırılır
        ...
        if hash_result is None:
            self.progress_update.emit(0) # Hata durumunda barı sıfırla
            return False

        try:
             # Hash sonucunu ayıkla
             parts = hash_result.split('  ')
             hash_value = parts[0].strip()
             
             # İmajın kaydedileceği dizini hazırla
             os.makedirs(os.path.dirname(target_path) or '.', exist_ok=True)
             
             image_filename = os.path.basename(target_path) 
             verification_hash_content = f"{hash_value}  {image_filename}\n" 

             # Hash değerini .sha512 dosyasına kaydet
             with open(hash_file_path, "w") as f:
                 f.write(verification_hash_content)
             self.output_update.emit(f"[{time.strftime('%H:%M:%S')}] SHA hash ({hash_value[:8]}...) dosyaya yazıldı.")
        ...
        # --- ADIM 2: DİSK KLONLAMA (Progress Bar Yüzdelik Modda) ---
        self.output_update.emit(f"[{time.strftime('%H:%M:%S')}] Adım 2/2: Disk klonlama (imaj oluşturma) başlatılıyor...")
        
        # Barı yüzdelik moda geri döndür
        self.progress_update.emit(0) 
        
        ddrescue_cmd = ['ddrescue', '-f', '-v', source_dev, target_path, self.log_file] 
        
        # Daha önce düzelttiğimiz progress parse eden ddrescue fonksiyonunu çağırır
        return self._execute_ddrescue_with_progress(ddrescue_cmd, "Disk İmajı Oluşturma", total_size)

    def _execute_ddrescue_flow(self, total_size):
...
class SystemBackupWizard(QMainWindow):
    ...
    def start_operation_sequence(self):
        """Asenkron işlem iş parçacığını başlatır ve ProgressBar'ı gerçek verilere göre ayarlar."""
        
        # ProgressBar'ı meşgul (0,0) moduna sokmak yerine 0-100 aralığına sabitliyoruz
        self.progress_bar.setRange(0, 100)
        self.progress_bar.setValue(0)
    ...
    def update_progress_bar(self, percentage):
        """Sinyale göre barı busy mod veya yüzdelik mod arasında değiştirir."""
        if percentage == -1:
            # Busy mode (sağa sola giden animasyon)
            self.progress_bar.setRange(0, 0)
        else:
            # Yüzdelik mode (normal dolum)
